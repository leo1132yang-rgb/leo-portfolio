  ...
  return <section className="profile-documentary" aria-label={cn ? "个人履历" : "Career Profile"}>
    ...
    <article className="profile-sequence profile-sequence--observation" id="profile-01">
      ...
      <div className="profile-sequence-copy">
        ...
        <p className="profile-sequence-year">2015</p><h2>{cn ? "摄影工作室 / 自我创作" : "Photography Studio / Personal Creation"}</h2>
        <p className="profile-role">{cn ? "摄影学习与个人创作阶段" : "Photography learning and personal creation"}</p>
        <p>{cn ? "进入摄影工作室学习，并持续进行个人创作。这一阶段让我开始真正理解画面、光线、人物与现场，也建立起最早的视觉观察能力。" : "I began learning in a photography studio while continuing personal work. It taught me to read images, light, people and live scenes, forming my earliest visual awareness."}</p>
        <div className="profile-words"><span>{cn ? "摄影学习" : "Photography"}</span><span>{cn ? "个人创作" : "Personal work"}</span><span>{cn ? "视觉观察" : "Visual observation"}</span></div>
      </div>
