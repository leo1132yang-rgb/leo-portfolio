...
export class Intro
{
    ...
    setText() {
        ...
        const ctx=canvas.getContext('2d');ctx.clearRect(0,0,1024,512);ctx.fillStyle='#fff7e5';ctx.textAlign='center';ctx.font='bold 112px "Microsoft YaHei",sans-serif';ctx.fillText('出发，去看看',512,245);ctx.font='46px "Microsoft YaHei",sans-serif';ctx.fillText('点击汽车 / 按回车',512,345);
        const map=new THREE.CanvasTexture(canvas);
        const mesh=new THREE.Mesh(new THREE.PlaneGeometry(5,2.5),new THREE.MeshBasicNodeMaterial({map,transparent:true,depthWrite:false}));this.label.add(mesh);
        this.text={mesh,textures:new Map([['zh',map]]),updateTexture:()=>{}};
    }
