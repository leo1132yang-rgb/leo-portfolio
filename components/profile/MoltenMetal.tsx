...
// React Bits official MoltenMetal (TS default); shader retained unchanged.
...
const MoltenMetal: React.FC<MoltenMetalProps> = ({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      ...
      antialias: false,
      dpr: Math.min(window.devicePixelRatio || 1, mobile.matches ? 1 : 1.5)
    });

    const gl = renderer.gl;
    partialCleanup.push(() => gl.getExtension('WEBGL_lose_context')?.loseContext());
    ...
    partialCleanup.push(() => { if (canvas.parentNode === container) container.removeChild(canvas); });

    const geometry = new Triangle(gl);
    partialCleanup.push(() => geometry.remove());
    ...
    partialCleanup.push(() => program.remove());
    if (!gl.getProgramParameter(program.program, gl.LINK_STATUS)) throw new Error('Shader initialization failed');
    ...
    const render = () => {
      ...
      catch { failed = true; canvas.style.opacity = '0'; onErrorRef.current?.(); }
    };
    ctxMap.set(container, { renderer, program, mesh, render });
    ...
    const loop = (t: number) => {
      if (failed) { raf = 0; return; }
      raf = requestAnimationFrame(loop);
      if (t - lastFrame < 1000 / (mobile.matches ? 24 : 30)) return;
      lastFrame = t;
    ...
    const tryStart = () => {
      if (!failed && !reduced.matches && isVisible && isPageVisible && raf === 0) raf = requestAnimationFrame(loop);
    };
    const tryStop = () => {
      if (raf !== 0) {
    ...
    const onMotion = () => {
      if (reduced.matches) { tryStop(); render(); } else tryStart();
    };
    const onContextLost = () => { failed = true; tryStop(); canvas.style.opacity = '0'; onErrorRef.current?.(); };
    ...
    return () => {
      ...
      partialCleanup.reverse().forEach(cleanup => cleanup());
    };
    } catch {
      ctxMap.delete(container);
      partialCleanup.reverse().forEach(cleanup => cleanup());
  ...
  useEffect(() => {
    ...
    // Draw the configured first frame even when reduced motion stops the RAF loop.
    ctx.render();
    ctx.renderer.gl.canvas.style.opacity = '1';
  }, [
    color1,
    color2,
    color3,
