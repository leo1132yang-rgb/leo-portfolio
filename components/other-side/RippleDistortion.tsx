...
type RippleTrigger = 'hover' | 'click';
...
export default function RippleDistortion({
  ...
  const uniformsRef = useRef<{
    wave: Record<string, { value: number }>;
  ...
  useEffect(() => {
    ...
    // Keep the local image and entry button usable without WebGL or motion.
    ...
    let compositeUniforms: Record<string, { value: number | number[] | Texture }>;
    ...
    compositeUniforms = {
      ...
      uDisplacement: {
        value: displacementTarget.texture
      },
      uResolution: {
        value: [1, 1]
      },
    ...
    const resize = () => {
      ...
      compositeUniforms.uResolution.value = [
        width,
        height
      ];
      ...
      compositeUniforms.uTexel.value = [
        1 / fieldW,
        1 / fieldH
      ];
    };
    ...
    const loop = (
      ...
      geometry.attributes.iOffset.needsUpdate =
        true;

      geometry.attributes.iScale.needsUpdate =
        true;

      geometry.attributes.iOpacity.needsUpdate =
        true;
    ...
    return () => {
      ...
      geometry.remove();
      compositeMesh.geometry.remove();
      waveProgram.remove();
      compositeMesh.program.remove();
      gl.deleteTexture(imageTexture.texture);
      displacementTarget.textures.forEach(texture => gl.deleteTexture(texture.texture));
      gl.deleteFramebuffer(displacementTarget.buffer);
  ...
  useEffect(() => {
    ...
    u.composite.uStrength.value =
      strength;

    u.composite.uSwirl.value =
      swirl;

    u.composite.uDispersion.value =
      dispersion;

    u.composite.uGlint.value =
      glint;

    u.composite.uTintAmount.value =
      tintAmount;

    u.composite.uGrayscale.value =
      grayscale ? 1 : 0;

    u.composite.uHighlight.value =
      hexToRGB(
        highlightColor
      );

    u.composite.uTint.value =
      hexToRGB(tint);
  }, [
    rings,
    strength,
  ...
  return (
    <div
      ...
      style={{ backgroundImage: `url("${src}")`, '--ripple-grayscale': grayscale ? 1 : 0, ...style } as CSSProperties}
    >{children}</div>
  );
}
