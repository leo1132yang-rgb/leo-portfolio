...
export function CareerProfile() {
  ...
  return <>
    ...
    <main>
      ...
      <footer className={styles.ending}><p>NOT A STRAIGHT LINE. A PRACTICE.</p><h2>THE PATH<br />WASN’T STRAIGHT.<br /><em>THAT’S THE POINT.</em></h2><p className={styles.endingCopy}>{cn ? <>路没有一直向前，<br />但它们最后汇到了一起。</> : <>Different directions.<br />Everything comes together.</>}</p><div className={styles.ctas}><Link href="/projects">{cn ? "查看项目作品" : "Explore projects"}<span>↗</span></Link><Link href="/other-side">{cn ? "进入我的世界" : "Enter Leo’s World"}<span>↗</span></Link><a href="mailto:leoyang1132@outlook.com">{cn ? "联系我" : "Contact me"}<span>↗</span></a></div><small>LEO / 李阳 <span>THE STORY CONTINUES.</span></small></footer>
    </main>
    </div>
  </>;
}
