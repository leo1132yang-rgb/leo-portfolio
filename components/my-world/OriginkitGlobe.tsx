...
export default function Globe({
    ...
    useEffect(() => {
        ...
        catch { setError("WebGL is unavailable on this device"); return; }
        ...
        const loadWorldData = async () => {
            try {
                ...
                const response = await fetch(
                    "/data/ne_50m_land.json", { signal: abort.signal }
                );
        ...
        const updateMarkers = () => {
            ...
            if (markerConfig.markers && markerConfig.markers.length > 0) {
                ...
                markerConfig.markers.forEach((marker) => {
                    ...
                    markerMesh.userData.placeId = marker.id;
        ...
        const animate = () => {
            if (disposed || document.hidden) { animationFrameId = null; return; }
        ...
        const handleMouseMoveDrag = (moveEvent: PointerEvent) => {
                if (moveEvent.pointerId !== activePointer) return;
                moved ||= Math.hypot(moveEvent.clientX - pressX, moveEvent.clientY - pressY) > 6;
        ...
        const handleMouseUp = (event: PointerEvent) => {
                ...
                if (!moved && event.type === "pointerup") selectMarker(event);
                if (canvas.hasPointerCapture(event.pointerId)) canvas.releasePointerCapture(event.pointerId);
                activePointer = null;
                isDragging = false;
        };
        canvas.addEventListener("pointerdown", handleMouseDown);
        canvas.addEventListener("pointermove", handleMouseMoveDrag);
        ...
        const selectMarker = (event: PointerEvent) => {
            ...
            // Screen-space tolerance makes small markers touchable; ocean raycast
            // rejects hidden/back-facing points. Coordinates are never nudged.
            scene.updateMatrixWorld(true);
            let closest: Mesh | null = null;
            let distance = event.pointerType === "touch" ? 22 : 12;
            for (const marker of markerMeshes) {
                const position = marker.getWorldPosition(new Vector3());
                const projected = position.clone().project(camera);
                const px = (projected.x + 1) * rect.width / 2 + rect.left;
                const py = (1 - projected.y) * rect.height / 2 + rect.top;
                const delta = Math.hypot(px - event.clientX, py - event.clientY);
                if (delta >= distance) continue;
                raycaster.setFromCamera(new Vector2(projected.x, projected.y), camera);
                const ocean = raycaster.intersectObject(oceanMesh)[0];
                if (ocean && ocean.distance + .025 < camera.position.distanceTo(position)) continue;
                distance = delta;
                closest = marker;
            }
            if (closest?.userData.placeId) onSelectRef.current?.(closest.userData.placeId);
        };
        ...
        const onVisibility = () => { if (!document.hidden) startAnimation(); };
        ...
        return () => {
            ...
            canvas.removeEventListener("pointermove", handleMouseMoveDrag);
            ...
            globeOutlineMesh?.geometry.dispose();
            if (globeOutlineMesh?.material instanceof MeshBasicMaterial) globeOutlineMesh.material.dispose();
            renderer.dispose();
